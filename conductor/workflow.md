...
- Production server: `root@<SERVER_IP>` → `/root/karusel_bot_v2` → container `karusel_bot_new`.
...
- Deploy only after user explicit "go ahead". Use the **rebuild sequence**:
  ...
  rtk sh -c 'cat <local_file> | ssh root@<SERVER_IP> "cat > /root/karusel_bot_v2/<remote_path>"'
  ...
  rtk ssh root@<SERVER_IP> 'cd /root/karusel_bot_v2 && docker compose down bot && docker compose up -d --build bot'
  # 3. Verify: imports resolve, polling started, no traceback.
  rtk ssh root@<SERVER_IP> 'cd /root/karusel_bot_v2 && docker compose logs --tail=100 bot'
  rtk ssh root@<SERVER_IP> 'cd /root/karusel_bot_v2 && docker compose exec bot python -c "from utils.states import TestRenderFlow; print(TestRenderFlow.waiting_for_text.state)"'
  ```
